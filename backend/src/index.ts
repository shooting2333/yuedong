  ...
  try {
    ...
    await pool.query(
      `INSERT INTO users (id, username, email, password_hash, coins)
       VALUES ($1, $2, $3, $4, $5)`,
      [userId, username, email, passwordHash, 100]
    );
...
// 创建宠物
app.post('/api/pets', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const petId = uuidv4();
    await pool.query(
      `INSERT INTO pets (id, user_id, name, personality, gender, current_form, trust_level)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [petId, userId, name, personality, gender, 'lazy_bug', 50]
    );
...
// 获取宠物信息
app.get('/api/pets/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query(
...
// 获取用户的宠物
app.get('/api/users/pets', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query(
...
// 开始战斗
app.post('/api/battles', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await pool.query(
      `INSERT INTO battles (id, user_id, monster_id, pet_id, status)
       VALUES ($1, $2, $3, $4, $5)`,
      [battleId, userId, monsterId, petId, 'in_progress']
    );
...
// 更新战斗进度
app.put('/api/battles/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isCompleted) {
      ...
      await pool.query(
        'UPDATE pets SET total_kcal = total_kcal + $1, trust_level = LEAST(100, trust_level + 5) WHERE id = $2',
        [totalKcal, battle.pet_id]
      );
...
app.post('/api/groups', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await pool.query(
      `INSERT INTO groups (id, creator_id, name, difficulty, required_kcal_per_person, coins_per_person, deadline)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [groupId, creatorId, name, difficulty, requiredKcalPerPerson, coinsPerPerson, deadline]
    );
...
// 宠物进化检查
const checkPetEvolution = async (petId: string) => {
  const pet = await pool.query('SELECT * FROM pets WHERE id = $1', [petId]);
  if (pet.rows.length === 0) return { evolved: false };
  ...
  if (evolution && total_kcal >= evolution.threshold && current_form !== 'shiny_rabbit') {
    await pool.query(
      'UPDATE pets SET current_form = $1 WHERE id = $2',
      [evolution.nextForm, petId]
    );
...
// 更新用户统计
const updateUserStats = async (userId: string, battleCompleted: boolean, kcalAdded: number) => {
  const today = new Date().toISOString().split('T')[0];
  const stats = await pool.query('SELECT * FROM user_stats WHERE user_id = $1', [userId]);

  if (stats.rows.length === 0) {
    await pool.query(
      `INSERT INTO user_stats (user_id, total_battles, total_wins, total_kcal, last_active_date, current_streak)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [userId, battleCompleted ? 1 : 0, battleCompleted ? 1 : 0, kcalAdded, today, 1]
    );
  } else {
    ...
    await pool.query(
      `UPDATE user_stats SET total_battles = total_battles + $1, total_wins = total_wins + $2,
       total_kcal = total_kcal + $3, last_active_date = $4, current_streak = $5 WHERE user_id = $6`,
      [battleCompleted ? 1 : 0, battleCompleted ? 1 : 0, kcalAdded, today, newStreak, userId]
    );
  }
};
...
const checkAchievements = async (userId: string) => {
  ...
  if (stats.rows.length === 0 || user.rows.length === 0) return [];

  const achievements = await pool.query('SELECT * FROM achievements');
  const unlockedIds = await pool.query(
  ...
  for (const achievement of achievements.rows) {
    if (unlockedIds.rows.some((u: any) => u.achievement_id === achievement.id)) continue;

    const req = achievement.requirement;
    let unlocked = false;

    if (achievement.type === 'battle' && stats.rows[0].total_battles >= req.battles) unlocked = true;
    if (achievement.type === 'exercise' && stats.rows[0].total_kcal >= req.total_kcal) unlocked = true;
    if (achievement.type === 'pet' && pets.rows.some((p: any) => p.trust_level >= req.trust_level)) unlocked = true;
    if (achievement.type === 'daily' && stats.rows[0].current_streak >= req.streak) unlocked = true;
    if (achievement.type === 'social' && user.rows[0].coins >= req.coins) unlocked = true;
...
// 获取用户统计
app.get('/api/users/stats', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const stats = await pool.query('SELECT * FROM user_stats WHERE user_id = $1', [req.user?.id]);
...
// 获取宠物详情
app.get('/api/pets/:id/detail', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
// 检查宠物进化
app.post('/api/pets/:id/check-evolution', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const pet = await pool.query(
      'SELECT * FROM pets WHERE id = $1 AND user_id = $2',
      [req.params.id, req.user?.id]
...
// 运动量排行榜
app.get('/api/leaderboard/kcal', async (req: Request, res: Response) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
    const offset = parseInt(req.query.offset as string) || 0;

    const leaderboard = await pool.query(
      `SELECT ROW_NUMBER() OVER (ORDER BY s.total_kcal DESC) as rank,
...
// 虚拟币排行榜
app.get('/api/leaderboard/coins', async (req: Request, res: Response) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
    const offset = parseInt(req.query.offset as string) || 0;

    const leaderboard = await pool.query(
      `SELECT ROW_NUMBER() OVER (ORDER BY u.coins DESC) as rank,
...
app.get('/api/achievements', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = achievements.rows.map((a: any) => ({
      ...a,
      unlocked: unlockedIds.rows.some((u: any) => u.achievement_id === a.id)
    }));
...
// 获取用户已解锁成就
app.get('/api/achievements/user', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const achievements = await pool.query(
...
// 检查成就
app.post('/api/achievements/check', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const newAchievements = await checkAchievements(req.user?.id || '');
