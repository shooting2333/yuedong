...
const insertSampleData = async () => {
    try {
        ...
        for (const monster of monsters) {
            await pool.query(`INSERT INTO monsters (name, type, required_kcal, reward_coins, reward_fragments, description)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT DO NOTHING`, [monster.name, monster.type, monster.required_kcal, monster.reward_coins, monster.reward_fragments, monster.description]);
        }
        // 插入装备
        const equipment = [
        ...
        for (const item of equipment) {
            await pool.query(`INSERT INTO equipment (name, type, rarity)
         VALUES ($1, $2, $3)
         ON CONFLICT DO NOTHING`, [item.name, item.type, item.rarity]);
        }
        // 插入成就
        const achievements = [
        ...
        for (const achievement of achievements) {
            await pool.query(`INSERT INTO achievements (name, description, type, requirement, reward_coins)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT DO NOTHING`, [achievement.name, achievement.description, achievement.type, JSON.stringify(achievement.requirement), achievement.reward_coins]);
        }
